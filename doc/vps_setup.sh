...
echo '[+] 正在安装基础依赖'
sudo apt-get install gcc git zsh python expect curl libdnet openssl wget gcc python-dev python-pypcap autoconf automake python3-dev python-gevent python-pip python-m2crypto libxml2-dev libxslt1-dev zlib1g-dev libmysqlclient-dev libxml2-dev libxslt1-dev mysql-server -y;
echo '[+] 正在从远程获取源代码'
...
if [ ! -f tomcat.tar.gz ]; then
  curl 'http://mirrors.hope6537.com:81/tomcat.tar.gz' >> tomcat.tar.gz;
fi
echo '[+] 完成Tomcat下载'
